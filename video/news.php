...
<body>
    ...
    <div class="content">
        ...
        <img src="img/news.png" alt="news" height="85px" class="newsman"><div class="manword">請密切關注活動相關最新資訊！</div>
            <table class="newstable">
                <thead>
                    <tr>
                        <td class="newsHead">標題</td><td class="newsHead">發布時間</td><td class="newsHead">點擊人次</td>
                    </tr>
                </thead>
                <tbody>
                    <?php
                        $get="SELECT `title`,`content`,`manager`,`time`,`num`,`count` FROM `krtco_news` ORDER BY `time` DESC";
                        $data=mysqli_query($con,$get)or die(mysqli_error());
                        ...
                        for($i=0;$i<100;$i++){
                            ...
                            if($check<15){
                            echo '<tr><td class="newsTitle"><form action="counts.php" method="post" target="my_iframe"><button class="md-trigger" data-modal="a'.$result["num"].'" 
                                        onclick="clickCount'.$result["num"].'()" name="test" style="font-size: 18px;"><input type="hidden" name="hidden" value="'.$result["count"].'"><input type="hidden" name="num" value="'.$result["num"].'">';
                            echo $result["title"];
                            echo '<img src="images/new.png" style="width:40px; float:right;"></button></form></td><td class="newsDate">';
                            echo $result["time"];
                            echo '</td><td class="newsCount"><input style="text-align:center;font-size:18px; color:black;" type="button" disabled="disabled" class="countTime" 
                                    id="'.$result["num"].'" name="count" value="'.$result["count"].'"></td>';
                            echo'<script>
                                    function clickCount'.$result["num"].'(){ 
                                        var count=document.getElementById("'.$result["num"].'").value;
                                        count++;
                                        document.getElementById("'.$result["num"].'").value=count;
                                        }
                                </script></tr><iframe name="my_iframe" src="counts.php" style="display:none"></iframe>';
                            }else{
                            echo '<tr><td class="newsTitle"><form action="counts.php" method="post" target="my_iframe"><button class="md-trigger" data-modal="a'.$result["num"].'" 
                                        onclick="clickCount'.$result["num"].'()" name="test" style="font-size: 18px;"><input type="hidden" name="hidden" value="'.$result["count"].'"><input type="hidden" name="num" value="'.$result["num"].'">';
                            echo $result["title"];
                            echo '</button></form></td><td class="newsDate">';
                            echo $result["time"];
                            echo '</td><td class="newsCount"><input style="text-align:center;font-size:18px; color:black;  type="button" disabled="disabled" class="countTime" 
                                    id="'.$result["num"].'" name="count" value="'.$result["count"].'"></td>';
                            echo'<script>
